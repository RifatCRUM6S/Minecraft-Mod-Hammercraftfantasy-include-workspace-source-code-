...
@EventBusSubscriber
public class OceanofsoulsTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(HammercraftfantasyModBlocks.OCEANOFSOULS_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("hammercraftfantasy:oceanofsouls_portal"), poiType);
	...
	public OceanofsoulsTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = HammercraftfantasyModBlocks.OCEANOFSOULS_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
