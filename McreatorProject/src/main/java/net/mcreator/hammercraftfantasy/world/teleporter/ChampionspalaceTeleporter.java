...
@EventBusSubscriber
public class ChampionspalaceTeleporter {
	public static Holder<PoiType> poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(HammercraftfantasyModBlocks.CHAMPIONSPALACE_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(ResourceLocation.parse("hammercraftfantasy:championspalace_portal"), poiType);
	...
	public ChampionspalaceTeleporter(ServerLevel level) {
		this.level = level;
	}
	...
	public Optional<BlockUtil.FoundRectangle> createPortal(BlockPos p_77667_, Direction.Axis p_77668_) {
		...
		BlockState blockstate = HammercraftfantasyModBlocks.CHAMPIONSPALACE_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
